"""Unit-level integration tests for crypto source compatibility layer."""
...
class TestSourceIntegration:
    def test_cryptofeed_config_normalizes_case(self):
    ...
    def test_ccxt_config_accepts_symbols(self):
        config = CCXTConfig(
            exchange="BINANCE",
            mode="trades",
            symbols=["BTC/USDT"],
        ...
        assert source._config.symbols == ["BTC/USDT"]

    def test_binance_s3_config_defaults(self):
        config = BinanceS3Config(bucket="bucket", prefix="prefix/")
        source = BinanceS3Source(config)
        assert source._config.datatype == "trades"
        assert source._config.unsigned is False

    def test_deprecated_kwargs_paths(self):
...
class TestRuntimeHooks:
    def test_ccxt_rate_limit_path(self, monkeypatch):
        class FakeExchange:
            rateLimit = 1000
        ...
        monkeypatch.setitem(sys.modules, "ccxt", types.SimpleNamespace(binance=FakeExchange))
    ...
    def test_cryptofeed_retry_path(self, monkeypatch):
